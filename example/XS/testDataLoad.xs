...
code("int xLocalDataDB = 0;");
code("int xLocalDataVar = 0;");
code("int loadNumHumans = 0;");
code("int localDataArray = 0;");
...
code("}");
code("}");
code("void addSavedDataQV(string qvName = \"\", int slot = 0, int size = 1) {");
...
code("xSetInt(dLocalData, xLocalDataSize, size);");
code("xSetInt(dLocalData, xLocalDataSlot, slot);");
code("xSetInt(dLocalData, xLocalDataDB, destDB);");
code("xSetInt(dLocalData, xLocalDataVar, destVar);");
code("recordTotalSize(slot, size);");
code("}");
...
code("for(i=0; < 16) {");
code("zSetInt(localDataArray, i, 0);");
code("}");
...
code("if (xGetInt(dLocalData, xLocalDataDB) == 0) {");
code("debugLog(xGetString(dLocalData, xLocalDataName));");
code("currentdata = trQuestVarGet(xGetString(dLocalData, xLocalDataName));");
code("} else {");
code("currentdata = xGetInt(xGetInt(dLocalData, xLocalDataDB), xGetInt(dLocalData, xLocalDataVar), p);");
code("}");
code("debugLog(\"currentdata: \" + currentdata);");
code("currentdata = xsMax(0, currentdata);");
code("currentdata = xsMin(currentdata, xGetInt(dLocalData, xLocalDataSize) - 1);");
code("zSetInt(localDataArray, slot, zGetInt(localDataArray, slot) * xGetInt(dLocalData, xLocalDataSize) + currentdata);");
code("}");
code("for(i=0; < 16) {");
code("trSetCurrentScenarioUserData(i, zGetInt(localDataArray, i));");
code("}");
code("}");
code("void loadRawData() {");
code("xsSetContextPlayer(0);");
code("for(i=0; < 16) {");
code("zSetInt(localDataArray, i, trGetScenarioUserData(i));");
code("}");
code("}");
code("void loadDataFromArray(int arrNum = 0, int p = 1) {");
code("int slot = xGetInt(dLocalData, xLocalDataSlot);");
code("int currentdata = zGetInt(arrNum, slot);");
code("int val = iModulo(xGetInt(dLocalData, xLocalDataSize), currentdata);");
code("zSetInt(arrNum, slot, currentdata / xGetInt(dLocalData, xLocalDataSize));");
code("if (xGetInt(dLocalData, xLocalDataDB) == 0) {");
code("if (trCurrentPlayer() == p) {");
code("trQuestVarSet(xGetString(dLocalData, xLocalDataName), val);");
code("}");
code("} else {");
code("xSetInt(xGetInt(dLocalData, xLocalDataDB), xGetInt(dLocalData, xLocalDataVar), val, p);");
code("}");
code("}");
...
code("xSetPointer(dLocalData, 1);");
code("for(i=xGetDatabaseCount(dLocalData); >0) {");
code("loadDataFromArray(localDataArray);");
code("xDatabaseNext(dLocalData);");
code("}");
code("}");
...
code("{");
code("xsDisableSelf();");
code("xsEnableRule(\"setup_local_data\");");
...
code("xLocalDataDB = xInitAddInt(dLocalData, \"database\");");
code("xLocalDataVar = xInitAddInt(dLocalData, \"variable\");");
code("localDataArray = zNewArray(mInt, 16, \"localData\");");
...
code("currentSwordsmanData = iModulo(32, zGetInt(localDataArray, i));");
code("zSetInt(localDataArray, i, zGetInt(localDataArray, i) / 32);");
...
code("}");
code("trQuestVarSet(\"example\", cNumberPlayers);");
...
code("rule setup_local_data");
code("inactive");
code("highFrequency");
code("{");
code("NEXT_TRIGGER_NAME = \"did_it_work\";");
code("addSavedDataQV(\"example\", 0, 12);");
